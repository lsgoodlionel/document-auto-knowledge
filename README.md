...
把文档的大纲、标题、正文和章节结构转换成可编辑的知识网络，并支持统一导入、历史项目沉淀、多格式导出、账户登录和后续知识编辑。
...
- 首页、登录页、知识网络页都接入统一导航壳层
- 站点启用强制登录：未登录访问首页或知识网络会自动跳转登录页
- 内置本地测试管理员账号：`admin / admin123`
- 提供独立“用户菜单”页面，可查看当前账号、会话状态、快捷入口并执行退出登录
- 知识网络左侧结构树支持显示级别切换：`1-2 级 / 1-3 级 / 全部`
- 左侧结构树支持展开同级、收起同级，并在大项目中自动保护首屏渲染
- 知识网络画布已升级为思维导图式画布，支持节点选中、拖拽、缩放、平移和折叠
- 目录总览与思维导图画布共享同一选中状态，点击任意节点会同步更新右侧节点编辑区
- 大型项目在 API 模式下只保存轻量视图状态，不再因浏览器 `sessionStorage` 超限导致编辑器打不开
- 节点可维护标题、正文 `note`、上下级关系和同级顺序
- 支持跨项目挂接：把一个项目的根节点或整棵根级树挂到另一个项目节点下
- 支持思维导图批量读写接口，保留节点坐标、折叠状态、样式和边关系模型
...
首页会显示统一品牌名“文档自动知识库系统”和统一导航栏。当前主线已经启用强制登录模式，未登录访问首页或知识网络页时会自动跳转到登录页。
...
登录后可以在首页选择支持格式中的任意文件，上传区会列出当前前端接受的扩展名。Word `.docx` 支持离线浏览器解析；PDF、电子书、图片、Excel、思维导图等格式需要通过本地服务导入，暂未支持或缺少依赖时会在页面上显示后端返回的结构化错误。

推荐验证主流程：

1. 打开 `http://127.0.0.1:8000` 并使用 `admin / admin123` 登录
2. 在首页上传文档并等待项目创建成功
3. 点击“进入知识网络”或在“历史项目”里直接打开项目
4. 在编辑器中查看左侧目录、思维导图画布和右侧节点正文
5. 按需要修改节点、挂接其他项目，再从顶部导出文件
6. 点击导航“用户菜单”查看当前账号、快捷入口和退出登录
...
- API 模式下，编辑器只持久化 `projectId`、当前选中节点和视图状态，避免超大项目把浏览器存储打满
...
- `png`：可用，当前为基础结构图像导出版本，依赖本机图像生成能力

认证与会话：

- `GET /api/auth/me`：读取当前登录用户
- `POST /api/auth/login`：使用用户名密码创建本地会话
- `POST /api/auth/logout`：清除本地会话
- 当前为本地单机最小可用版本，默认账号为 `admin / admin123`

思维导图接口：

- `GET /api/projects/{id}/mindmap`：读取项目的思维导图节点和边信息
- `PUT /api/projects/{id}/mindmap`：批量保存思维导图树、节点样式和边关系
...
当前 `main` 已经收敛了前面的 V1、V2、V2.1 和 V3 主线结果，现状可以概括为：

- 多格式导入框架已完成并接入主线
- 节点正文 `note` 入库、编辑和 Word 导出回写已完成
- 左侧目录总览、思维导图画布、右侧节点编辑区已联动
- 跨项目挂接、统一导出面板和本地安装包已完成
- 统一导航、登录页、强制登录模式和用户菜单页已完成并接入主线

后续继续开发时，请优先以 `main` 为唯一主线，并先阅读：
