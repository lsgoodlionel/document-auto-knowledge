...
```text
7c5a798 Allow per-window development ports
```
...
- `frontend/editor.js` 仍主要使用浏览器状态，尚未完全接入数据库 CRUD。
- 首页还没有完整历史项目列表。
- Word 导入暂未把标题下正文完整归入节点 note。
- 安装包需要跟随新版后端服务启动方式继续打磨。
- 本地 `main` 比 `origin/main` 超前 1 个提交，需要后续推送。

## 下一步建议

优先顺序：

1. 窗口 A 完整化后端 API，尤其是项目删除、重命名、节点排序。
2. 窗口 B 把编辑器保存、新增、删除接入 API。
3. 窗口 C 做首页历史项目列表。
4. 窗口 D 提升 Word 正文导入和导出样式。
5. 窗口 E 更新安装包和 GitHub Actions。
