...
<map version="1.0.1">
  <node TEXT="Root">
    ...
    def test_mindmap_service_bootstraps_required_tables(self) -> None:
        with db.connect() as conn:
            project_id = projects.create_project(conn, "Mindmap bootstrap")
            conn.commit()
        mindmap.get_project_mindmap(project_id)
        with db.connect() as conn:
            tables = {
                row["name"]
...
class HttpApiSmokeTest(unittest.TestCase):
    ...
    def test_mindmap_api_reads_and_writes_enhanced_structure(self) -> None:
        ...
        status, payload, _ = self.request("GET", f"/api/projects/{project_id}/mindmap")
        self.assertEqual(status, 200)
